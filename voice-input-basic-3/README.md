...
如果您的設備編號不同（例如 `hw:0,0`），請修改 `voice_recorder.py` 和 `voice_daemon.py` 中的相關配置（現在預設已改為動態路徑，僅需修改硬體 ID）。
...
```text
# 設定 $mod+v 為觸發語音輸入 (請將 [PATH] 替換為實際專案絕對路徑)
bindsym $mod+v exec --no-startup-id [PATH]/toggle_voice.sh
```

您可以使用 `pwd` 獲取當前目錄的絕對路徑。
